    ...
    <section class="grid">
      <For keyed each={devices()}>{device => <article>
        ...
        <div class="capabilities">
          <For keyed each={Object.keys(device.capabilities)}>{id => {
            const capability = device.capabilities[id]
            return <div class="row"><span>{id}</span><strong>{String(capability.value)}{capability.kind === "number" ? capability.unit ?? "" : ""}</strong></div>
          }}</For>
        </div>
